...
namespace Ants
{
    ...
    class Explorer : Bot
    {
        ...
        private bool NormalMove(IGameState state, Ant ant)
        {
            ...
            if (direction == Direction.Noone && state.EnemyAnts.Count > 0) direction = AtkEnemy(state, ant);
        ...
        public Direction AtkEnemy(IGameState state, Ant ant)
        {
            ...
            {
                nearmyant.Clear();
                foreach (Ant myant in state.MyExpAnts)                                                //----------------------------------------------Nur exp ants
                {
                    if (state.GetDistance(myant, enemy) <= 36) nearmyant.Add(myant);
                }
                if (nearmyant.Count > nearenemyant.Count)
                {
                    Direction direction;
                    direction = DirectionToTarget(state, 365, ant, (int)Tile.EnemyAnt, (int)Layer.Tiles);
                    if (direction != Direction.Noone) return direction;
                }
                if (nearmyant.Count <= nearenemyant.Count)
                {
                    state.map[state.test_x(ant.Row - (enemy.Row - ant.Row)), state.test_y(ant.Col - (enemy.Col - ant.Col)), (int)Layer.Special] = 100;
                    Direction direction;
        ...
        public Direction AtkEnemy(IGameState state, Ant ant)
        {
            ...
            foreach (Ant myant in state.MyExpAnts)                                                //----------------------------------------------Nur exp ants
            {
                if (state.GetDistance(ant, myant) <= 5) nearmyant.Add(myant);
            }
            foreach (Ant enemy in nearenemyant)
            {
                if (nearmyant.Count > nearenemyant.Count)
                {
                    state.map[enemy.Row,enemy.Col,(int)Layer.Special] = 100;
                    Direction direction;
                    direction = DirectionToTarget(state, 365, ant, 100, (int)Layer.Special);
                    if (direction != Direction.Noone) return direction;
                    state.map[enemy.Row,enemy.Col,(int)Layer.Special] = 0;
                }
                if (nearmyant.Count <= nearenemyant.Count)
                {
                    state.map[state.test_x(ant.Row - (enemy.Row - ant.Row)), state.test_y(ant.Col - (enemy.Col - ant.Col)), (int)Layer.Special] = 100;
                    ...
                    direction = DirectionToTarget(state, 365, ant, 100, (int)Layer.Special);
                    if (direction != Direction.Noone) return direction;
                    state.map[state.test_x(ant.Row - (enemy.Row - ant.Row)), state.test_y(ant.Col - (enemy.Col - ant.Col)), (int)Layer.Special] = 0;
                }
            }
            return Direction.Noone;
        }
