...
namespace Ants
{
    public static class Pathfinding_alt
    {
            ...
            while (foundway == false)
            {
                ...
                if (myAnt != null)
                {
                    ...
                    else // Falls ein Knoten ein Ant ist welcher bewegt wurde diesen Punkt aus Liste entfernen um Kollision zu verhindern
                    {
                        myKnoten.RemoveAt(z);
                        lastindex--;
                        if (z <= lastindex) { } // Nur weiterfahren falls es noch einen Knoten hat
                        else return false;
                    }
                }
                else
                {
                    if (state.map[state.test_x(x + 1), y, (int)Layer.Tiles] != (int)Tile.Water)
                    {
                        ...
                        {
                            if (knotenschonvorhanden(myKnoten, state.test_x(x + 1), y, lastindex) == false)
                            {
                    ...
                    if (state.map[x, state.test_y(y + 1), (int)Layer.Tiles] != (int)Tile.Water)
                    {
                        ...
                        {
                            if (knotenschonvorhanden(myKnoten, x, state.test_y(y + 1), lastindex) == false)
                            {
